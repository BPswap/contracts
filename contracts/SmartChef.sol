...
// added functionnalities by the bpswap's devs
//
...
contract SmartChef is Ownable {
    ...
    // The block number when MTN mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 mtnReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Withdraw MTN tokens from STAKING.
